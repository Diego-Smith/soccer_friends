...
create table "CATEGORY" ("ID" BIGINT GENERATED BY DEFAULT AS IDENTITY(START WITH 1) NOT NULL PRIMARY KEY,"USERNAME" VARCHAR NOT NULL);
create table "CONFIGURATION" ("ID" BIGINT GENERATED BY DEFAULT AS IDENTITY(START WITH 1) NOT NULL PRIMARY KEY,"ID_USER" VARCHAR NOT NULL,"ID_INTEREST" VARCHAR NOT NULL,"CREATION_TIME" TIMESTAMP NOT NULL);
create unique index "CONF_UNIQUE" on "CONFIGURATION" ("ID_USER");
create table "FRIENDSHIP" ("ID" BIGINT GENERATED BY DEFAULT AS IDENTITY(START WITH 1) NOT NULL PRIMARY KEY,"ID_USER_A" BIGINT NOT NULL,"ID_USER_B" BIGINT NOT NULL);
create unique index "uniqueFriendshipIndex" on "FRIENDSHIP" ("ID_USER_A","ID_USER_B");
create table "INTEREST" ("ID" BIGINT GENERATED BY DEFAULT AS IDENTITY(START WITH 1) NOT NULL PRIMARY KEY,"NAME" VARCHAR NOT NULL,"CREATED_BY_USER_ID" BIGINT NOT NULL,"ID_CATEGORY" BIGINT NOT NULL);
create table "PAGE_VISITED" ("ID" BIGINT GENERATED BY DEFAULT AS IDENTITY(START WITH 1) NOT NULL PRIMARY KEY,"PAGE_NAME" VARCHAR NOT NULL,"IP" VARCHAR NOT NULL,"DATE" TIMESTAMP NOT NULL,"ID_USER" BIGINT);
create table "USER" ("ID" BIGINT GENERATED BY DEFAULT AS IDENTITY(START WITH 1) NOT NULL PRIMARY KEY,"USERNAME" VARCHAR NOT NULL,"PASSWORD" VARCHAR NOT NULL,"NAME" VARCHAR,"SURNAME" VARCHAR,"AUTH_METHOD" VARCHAR NOT NULL,"PROVIDER_ID" VARCHAR NOT NULL);
create unique index "UNIQUE_USERNAME" on "USER" ("USERNAME");
create table "USER_INTEREST" ("ID" BIGINT GENERATED BY DEFAULT AS IDENTITY(START WITH 1) NOT NULL PRIMARY KEY,"ID_USER" BIGINT NOT NULL,"ID_INTEREST" BIGINT NOT NULL,"CREATION_TIME" TIMESTAMP NOT NULL);
create unique index "UIT_UNIQUE" on "USER_INTEREST" ("ID_USER","ID_INTEREST");
create table "USER_OAUTH2INFO" ("USER_ID" BIGINT NOT NULL PRIMARY KEY,"ACCESS_TOKEN" VARCHAR NOT NULL,"TOKEN_TYPE" VARCHAR,"EXIPIRES_IN" INTEGER,"REFRESH_TOKEN" VARCHAR);
